...
{
	__asm {
		move a1,a0					// a1 = retries
		loadi	a0,0xE0				// set interrupt mask level sixty-three
		csrrs	a0,0x3004,a0	// disable all interrupts
		sync
		move a2,a1
		loadi a1,1					// system task handle
.loop:
		add. a2,a2,-1				// decrement retries
		beq cr0,.timeout
		amoswap. r0,a1,OSSEMA
		bne cr0,.loop				// semaphore set?
		csrrw r0,0x3004,a0	// restore interrupt level
		loadi a0,1
		blr
.timeout:
		csrrw r0,0x3004,a0
...
private hTCB SelectTaskToRunHelper(int nn)
{
	int kk;
  hTCB h, h1;
	TCB *p, *q;
